...
def calc_norm(path_dir_list=Path("./list"),
    ...
    list_path_wav = []
    list_txt = (path_dir_list / "train").glob("*.txt")
    for path_txt in list_txt:
        with open(path_txt, "r", encoding="utf_8") as txt:
            list_path_wav.extend(txt.read().splitlines())

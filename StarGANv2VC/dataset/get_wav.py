...
limit_num_file = {
    "train": 100,
    "valid": 10}
list_lacker = ["jvs006", "jvs028", "jvs037", "jvs058"]
...
def get_wav(path_jvs,
    ...
    for use, set in dict_set.items():
        ...
        for path_wav_read in tqdm(list_path_wav):
            sr_file, wav = iowav.read(path_wav_read)

            path_dir_wav_sp = path_data / use / f"{path_wav_read.parents[2].name}"
            path_dir_wav_sp.mkdir(exist_ok=1)
            path_wav_write = path_dir_wav_sp / path_wav_read.name
